...
- **`lib/panel.ts`** — the Shadow-DOM modal over x.com, carrying its own copy of the `--xg-*` tokens. `openShell()` builds the chrome (overlay, header, Escape/⌘↵, dismissal) and every entry point renders into it: `openPanel()` for the composer's ✨ button, `openPolishPanel()` for the composer's pencil, and `openRemixPanel()` for the remix button on any tweet. All three gate on `loadReady()`, which enforces the account-then-key onboarding order in one place. This is the only surface that still renders UI on x.com, and it stays React-free on purpose — the content script is ~46kB precisely because React and Tailwind are not in it.
...
- **Polish** — a pencil button left of the ✨, hidden until the composer actually has a draft (`readEditorText()` reads Draft.js's `[data-block]` divs, so blank lines survive). It is the only flow whose input is the user's own writing, which is why it skips everything the reply path does: no screening, no `applyStyle` (it would "fix" the user's own casing habits), and `POLISH_SAMPLING` instead of `HUMAN_SAMPLING` — the frequency/presence penalties exist to push a *fresh* generation off familiar phrasing, and here that phrasing is the thing being preserved. One call returns two versions, a minimal correction and a tightened one.
